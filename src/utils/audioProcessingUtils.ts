...
export const processAudioForCompatibility = async (audioBlob: Blob): Promise<ProcessedAudio> => {
  console.log("Processing audio for compatibility...", audioBlob.type, audioBlob.size);
  ...
  try {
    ...
    // Check if the audio already meets our requirements - accept either MP3 or WebM
    const isHighQuality = (
      (metadata.format === 'mp3' || metadata.format === 'webm') &&
      (metadata.sampleRate === undefined || metadata.sampleRate >= 16000) &&
      audioBlob.size > 1100 // Make sure it's not the tiny fallback blob
    );
...
<<<<<<< HEAD
    // Only attempt conversion if not WebM
    if (metadata.format !== 'webm') {
      ...
      // For WebM, just use as is
      console.log("Using WebM format directly");
      const dataUrl = await blobToDataURL(audioBlob);
...
>>>>>>> f14d5d6eebc8be0d4010cd62c07e1a7dd83042b8
  } catch (error) {
